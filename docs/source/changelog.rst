...
- Replaced the custom biped's estimated actuator armature with the
  ...
  gains from a 5 Hz natural frequency and 1.0 damping ratio. The source MJCF now
  carries the same reflected armature used by the training actuator configuration,
  so standalone MuJoCo simulations and training compile the same joint inertia.
